function t = customImReadFcn(Xfilename, Yfilename)
...
X = extractdata(processImagesMNIST(Xfilename));
Y = processLabelsMNIST(Yfilename);

t = table(squeeze(num2cell(X, [1,2,3])),double(Y), 'VariableNames', {'Predictors', 'Response'});
end
