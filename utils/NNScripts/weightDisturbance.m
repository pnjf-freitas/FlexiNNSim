...
for i = 1 : size(weights, 1)
    layerWeights = weights.Value{i};    
    layerGradientsSignMatrix = sign(gradients.Value{i});
    
    if (DisturbanceStruct.ExecutionEnvironment == "auto" && canUseGPU) || DisturbanceStruct.ExecutionEnvironment == "gpu"
        layerWeights = gpuArray(layerWeights);
        layerGradientsSignMatrix = gpuArray(layerGradientsSignMatrix);
    end
